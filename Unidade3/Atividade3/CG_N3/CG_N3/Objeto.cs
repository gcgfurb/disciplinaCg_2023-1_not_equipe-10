...
namespace gcgcg
{
    internal class Objeto
    {
      ...
      Console.WriteLine(" .. ERRO de Render - escolha OpenGL ou DirectX !!");
...
#endif
        internal int BuscarOVerticeMaisProximo(float deltaX, float deltaY)
        {
            var menorDistancia = float.MaxValue;
            var indice = -1;
            for (int i = 0; i < pontosLista.Count; i++)
            {
                var distancia = GetDistancia(deltaX, deltaY, pontosLista[i]);
        ...
        internal float GetDistancia(float x, float y, Ponto4D pontoB)
        {
            return GetDistancia(new Ponto4D(x, y), pontoB);
        }

        internal float GetDistancia(Ponto4D pontoA, Ponto4D pontoB)
        {
            var x1 = pontoA.X;
            var x2 = pontoB.X;
